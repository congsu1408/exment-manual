...
## v6.2.7 (2025/10/08)
1. 機能修正
    - Laravel.log にパフォーマンス調査向けのログ出力を追加  詳細は[こちら](/ja/system_setting?id=%e6%93%8d%e4%bd%9c%e3%83%ad%e3%82%b0%e3%82%92%e6%9c%89%e5%8a%b9%e3%81%ab%e3%81%99%e3%82%8b)
    - 集計ビューにフィルタ機能を追加
1. 不具合修正
    - ワークフローに関する不具合修正
    - 事象発生手順：
        ...
        2. 別画面でWFを進める
        3. 未更新の画面ですでに実行済のWFボタン押下  
        ⇒ 特にチェックもなく実行できてしまう


## v6.2.6 (2025/07/24)
1. 機能修正
    - Microsoft Entra のSAMLでSSO認証(Authenticator)できるように修正
    - カスタムテーブルの複製時にビューやフォーム複製できるように修正


## v6.2.5 (2025/06/19)
1. 機能修正
    - フィルタにコメントの検索を追加
    - 複合ユニークキーのキー列にparent_idを追加
1. 不具合修正
    - 子テーブルに閲覧権限が引き継がれない不具合の修正
    - フォームの削除ができない不具合の修正


## v6.2.4 (2025/05/21)
1. 機能修正
    - APIの上限数のパラメータをenvファイルから参照できるように修正
    - 操作ログのフィルタに作成日時を追加
    - カレンダーオブジェクト(Fullcalendar)を、スクリプトプラグインでカスタマイズできるよう改修
    - PHPStan Level 6対応
    - ユニットテストからSQL Serverを除外する対応
1. 不具合修正
    - 集計ビューに関する不具合の修正
...
## v6.2.3 (2025/03/18)
1. 機能修正
    - フィルタ検索時に、「更新ユーザ名」と「作成ユーザ名」の項目を表示し検索できるように修正
1. 不具合修正
    - カスタムフォーム設定の「初期値」設定が機能しない不具合の修正
    - カスタムテーブル削除時にテーブルに紐づく通知が一緒に削除されないケースがある不具合の修正
    - カレンダービューを含むテーブルをテンプレートに出力すると「終了日」がIDのまま出力される不具合の修正
    - カスタムバリューの関数「setValueStrictly」からの呼び出しの時のみ、カスタムテーブルの関数「validatorUnnecessaryColumn」が正しく機能しない不具合の修正


## v6.2.2 (2025/02/27)
1. 機能修正
    - ログイン設定でシングルサインアウト(Single Logout)を指定できるように設定項目を追加
    - SSOログイン後にログアウトする際、シングルサインアウトの指定がある場合、Exmentログイン画面にリダイレクトするように修正
　　　 ※Oktaは対象外です


## v6.2.1 (2025/02/12)
1. 機能修正
    - 役割グループ設定のインポート/エクスポートができるように修正
    - データ更新設定で、空欄や未選択に更新できるように修正
    - カスタムテーブル別の「行クリック時に編集画面遷移」ができるように修正
    - カスタムフォームに子テーブルを表示する場合に表示順を指定できるように修正
1. 不具合修正
    - フリーワード検索時のエラーの不具合修正 (laravel-admin)
    - ページあたり行数を変更すると特定の条件でエラーの不具合修正 (laravel-admin)
    - エクスポートのエラー（テーブル表示名に特定文字の場合）について不具合の修正
    - 複数選択可の画像orファイルのインポートでエラーになる不具合の修正
1. その他
    - JANバーコード対応で、JSバージョンを v0.21.2 → v0.21.3 へアップデート、一部テキスト誤字修正
    - カレンダーのJSバージョンをアップデートおよびアップデートに合わせた修正


## v6.2.0 (2024/12/24)
1. 機能追加
    - JANバーコード機能の追加
    - 請求書PDFメール送信プラグイン(標準プラグイン)  詳細は[こちら](https://github.com/exment-git/plugin-product/tree/main/document/PluginInvoiceDocument)
1. 不具合対応
    - CRUDプラグインでフリーワード検索を追加しても検索の対象にならない不具合の修正
    - 詳細画面での「custom_value_(テーブル名)」クラスの位置について不具合の修正


## v6.1.8 (2024/12/04)
1. 不具合対応
    - ワークフローのステータスを対象とした集計ビューにてSQLエラー発生する不具合の修正
    - ダッシュボードのエディタにアップロードしたファイルが翌日になると表示されない不具合の修正
    - 親テーブルの詳細画面で複数の子テーブルをページングするとエラーが発生する不具合の修正 (laravel-admin)
1. その他
    - API設定で、Composerエラーのときの注意コメントを追加　詳細は[こちら](/ja/system_setting?id=api%e3%82%92%e4%bd%bf%e7%94%a8%e3%81%99%e3%82%8b)


## v6.1.7 (2024/11/12)
1. 不具合対応
    - 対象テーブルを参照している他テーブルの列が検索インデックスでない場合に集計ビューで他テーブルの列を選べないようにするように修正
    - 公開フォームからファイル列にアップロードできない不具合の修正
    - 値が複数ある場合の「存在しない」という不具合の修正
    - ログイン関連フォームのinput要素の属性指定の入力タイプをテキストに修正
    - パスワードリセットを行おうとすると505エラーになる不具合の修正
    - 通知アクション設定で送信先の組織が保存されない不具合の修正
    

## v6.1.6 (2024/10/11)
1. 不具合対応
    - v6.1.5アップデート後、ファイル列にファイルがアップロードできなくなった不具合の修正
...
- v5.0.12 もリリースしています。[こちら](/ja/release_note?id=v5012-20241010)

1. 脆弱性対応
    - クロスサイトスクリプティングの脆弱性対応。詳細は[こちら](/ja/weakness/20241010)
    - 権限のないカスタムテーブルの設定変更できてしまう脆弱性対応。詳細は[こちら](/ja/weakness/20241010_2)


## v6.1.4 (2024/10/02)

1. 機能修正
    - laravel-admin PHPStan Level 5 & 6対応 (laravel-admin)
    - システム詳細設定の「ユーザービューを使用する」がYESの場合でも、特定のテーブルでユーザービューを使用できなくする設定値の追加

1. 不具合修正
    - composerのLDAPモジュールのインストールでエラーが発生する不具合の修正
    - 集計ビューで「集計データの明細を表示する」を選択するとエラーになる不具合の修正


## v6.1.3 (2024/09/12)

1. 不具合修正
    - v5.0.8 からv6.0.4へ移行したテーブルでフォームの編集が出来ない不具合の修正 (laravel-admin)
    - 通知設定編集画面の送信先ユーザーの表示不具合の修正
    - フォームの子テーブルを「テーブル形式にする」にした際の列種類「選択肢」の入力欄の幅が行を増やすたびに増えていく不具合の修正
    - プラグインの実施トリガーの日本語名称が「ワークフロース」になっている不具合の修正
...
1. 全般
    ...
    - MySQLの最低バージョンを、MySQL5.7から8.0以上へ変更(推奨：MySQL8.4)


## v5.0.13 (2024/10/11)
1. 不具合対応
    - v5.0.12アップデート後、ファイル列にファイルがアップロードできなくなった不具合の修正
